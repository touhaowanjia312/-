...
class MultiExchangeClient:
        ...
        try:
            ...
            if exchange_type == 'lbank':
                if hasattr(account, 'manual_contract_balance') and account.manual_contract_balance > 0:
                    ...
                    # 回退到现货余额
                    balance = client.fetch_balance({'type': 'spot'})
            ...
            try:
                ...
                try:
                    futures_balance = client.fetch_balance({'type': 'future'})
                    futures_amount = futures_balance['free'].get(currency, 0.0)
    ...
    def get_balance_detailed(self, account_name: str, currency: str = 'USDT') -> Optional[Dict[str, float]]:
        ...
        try:
            ...
            # 🔧 修复：明确获取现货余额
            try:
                spot_balance = client.fetch_balance({'type': 'spot'})
                ...
                # 如果获取现货失败，尝试默认方式
                try:
                    default_balance = client.fetch_balance()
        ...
        try:
            ...
            if exchange_type == 'bitget':
                ...
                try:
                    ...
                    client.set_position_mode(False, params={'productType': 'USDT-FUTURES', 'marginCoin': 'USDT'})
                except Exception as e:
                ...
                try:
                    ...
                    if '40774' in error_str:
                        ...
                        for i in range(retries):
                            ...
                            try:
                                success = client.create_market_order(contract_symbol, side, try_amount, params=params)
                                amount = try_amount
                                break
                            except Exception as e2:
    ...
    def close_position(self, account_name: str, symbol: str) -> bool:
        ...
        try:
            ...
            positions = client.fetch_positions([contract_symbol], params=fetch_params)
    ...
    def list_open_positions(self, account_name: str) -> List[Dict[str, Any]]:
        ...
        try:
            params = {'productType': 'USDT-FUTURES', 'marginCoin': 'USDT'} if exchange_type == 'bitget' else {}
            positions = client.fetch_positions(params=params)
    ...
    def fetch_order_status(self, account_name: str, symbol: str, order_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            contract_symbol = self._convert_to_contract_symbol(client, symbol)
            order = client.fetch_order(order_id, contract_symbol)
    ...
    def cancel_open_reduce_only_orders(self, account_name: str, symbol: str) -> int:
        ...
        try:
            contract_symbol = self._convert_to_contract_symbol(client, symbol)
            params = {'productType': 'USDT-FUTURES', 'marginCoin': 'USDT'} if exchange_type == 'bitget' else {}
            open_orders = client.fetch_open_orders(contract_symbol, params=params)
            for o in open_orders or []:
                try:
                    ...
                    if reduce_only:
                        client.cancel_order(o.get('id'), contract_symbol, params=params)
                        cancelled += 1
        ...
        try:
            ...
            if exchange_type == 'bitget':
                try:
                    ...
                    import time
                    # 计算持仓方向：对多仓止损需关闭多仓 → holdSide=long；空仓相反
                    # Bitget API 文档显示 holdSide 是必需参数
                    ...
                    body = {
                        ...
                        'executePrice': '0',  # 市价执行
                        'holdSide': hold_side,  # Bitget API 文档显示这是必需参数
                        'size': str(amount),
                    ...
                    if response is None:
                        ...
                        if response is None:
                            try:
                                ...
                                # 如果返回 400 错误，可能是参数问题，尝试移除 executePrice 或调整参数
                                if error_status == 400:
                                    logger.debug(f"{account_name} 尝试调整 TPSL 参数（移除 executePrice）")
                                    ...
                                    try:
                                        ...
                                        response = resp2.json()
                                        logger.debug(f"{account_name} TPSL 移除 executePrice 后成功: {response}")
                                    except Exception as fe2:
                                        logger.error(f"{account_name} TPSL 移除 executePrice 后仍失败: {fe2}")
                                        raise http_err
                                else:
                                    raise http_err
