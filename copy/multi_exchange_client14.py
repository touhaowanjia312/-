                        ...
                        if response is None:
                            try:
                                ...
                                if error_status == 400:
                                    ...
                                    # 如果所有 fallback 都失败了，记录警告但不返回None（让系统继续运行）
                                    if response is None:
                                        logger.warning(f"{account_name} Bitget TPSL 不可用，请通过发送止损信号手动设置止损")
                                        # 返回特殊状态，让trade_executor知道这是手动止损模式
                                        return {
                                            'status': 'manual_required',
                                            'message': 'Bitget TPSL不可用，请手动设置止损',
                                            'price': stop_price,
                                            'amount': amount
                                        }
