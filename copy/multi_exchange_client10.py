                            ...
                            try:
                                ...
                                if error_status == 400:
                                    ...
                                    if (
                                        ...
                                        logger.error(f"{account_name} 检测到 holdSide 相关错误，开始 fallback 尝试")
                                        holdside_attempts = []
                                        ...
                                        for hold_side_val in holdside_attempts:
                                            ...
                                            try:
                                                ...
                                                logger.error(f"{account_name} holdSide='{hold_side_val}' 失败: {e}")
                                                continue
                                    
                                    # 如果所有 fallback 都失败了，记录错误但不重新抛出异常
                                    if response is None:
                                        logger.error(f"{account_name} 所有 TPSL fallback 尝试都失败")
