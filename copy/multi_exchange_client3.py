                    ...
                    if response is None:
                        ...
                        if response is None:
                            try:
                                ...
                                # 如果返回 400 错误，可能是参数问题，优先处理 size 精度（40808），再尝试移除 executePrice
                                if error_status == 400:
                                    ...
                                    if response is None and (("holdSide error" in (error_detail or "")) or ("\"code\":\"43011\"" in (error_detail or ""))):
                                        try:
                                            body_no_hold = body.copy()
                                            body_no_hold.pop('holdSide', None)
                                            body_str = json.dumps(body_no_hold, separators=(',', ':'))
                                            message = timestamp + method + request_path + body_str
                                            signature = base64.b64encode(hmac.new(client.secret.encode('utf-8'), message.encode('utf-8'), hashlib.sha256).digest()).decode('utf-8')
                                            headers['ACCESS-SIGN'] = signature
                                            resp2 = requests.post(url, headers=headers, data=body_str, timeout=10)
