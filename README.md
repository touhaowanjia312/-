...
- **组件**
  - `TelegramSignalBot`：监听消息 → 解析信号 → 调用 `TradeExecutor`
  - `TradeExecutor`：统一执行器（单/多交易所）
  - `multi_exchange_client` / `exchange_client`：下单/撤单/查单/持仓/价格
  - `order_manager.PositionManager`：登记活跃持仓（含 trade_id/leverage），追踪/保本
  - `risk_manager`：风控准入（开仓前）与交易统计（开/平仓时）
  - `database.trading_db`：trades/orders/daily_stats/risk_events
  - GUI：展示统计、风险、历史与日志

- **主要流程**
  1. 解析消息 → 结构化信号 `TradingSignal`
  2. `TradeExecutor` 生成订单计划 → 风控校验
  3. 入场（市价/限价）→ 记录订单与 `trade_id`
  4. 设置初始止损（±4% 缺省）与（价格型或回退）止盈
  5. 登记持仓信息（entry/size/side/sl/tps/trailing/breakeven/trade_id/leverage）
  6. 背景任务：
     - TP1 成交 → 止损移至保本位
     - 仓位归零 → 自动 `close_trade` 计算真实 PnL、更新风控与清理内存持仓


## 配置速查表（关键开关）

- **交易运行**
  - `TRADING_ENABLED`：False=监听不下单；True=实际下单
  - `EXCHANGE_TESTNET`：True=测试网；False=实盘
- **风控与仓位**
  - `RISK_PERCENTAGE`：单笔风险占余额比例
  - `MAX_POSITION_SIZE`：最大仓位数量
- **多交易所（exchanges_config.json）建议项**
  - `default_leverage`、`enabled`、`testnet`
  - `use_margin_amount`（固定保证金）、`margin_amount`
  - `risk_as_notional`（风险按名义）
- **Telegram**
  - `TELEGRAM_API_ID/HASH/PHONE`，`TELEGRAM_GROUP_ID`


## 结构化日志字段说明（节选）

- `exec_start`：mode(single/multi), symbol, signal_type, leverage
- `risk_blocked`：account, symbol, reason, tv
- `entry_order_placed`：account, symbol, side, amount, price, order_id
- `sl_placed`：account, symbol, side, amount, stop_price
- `tp_placed`/`tp_fallback_placed`：account, symbol, idx, portion, amount, price
- `tp1_order_id`：account, symbol, order_id
- `position_registered`：account, symbol, entry_price, size, trade_id
- `trade_recorded`：account, trade_id, symbol, entry_price, size, leverage
- `trade_closed_signal`：account, symbol, pnl


## 交易所适配要点速记

- 合约优先 `:USDT` 符号，`options.defaultType='future'`
- 止损/止盈单：优先 `reduceOnly`；Bitget 走专用 TPSL 接口参数（已在多交易所客户端适配）
- `stopLossPrice` 与 `trigger*` 参数不要混用；必要时携带 `productType/marginCoin`
- `fetch_positions` 字段名各所不同：`contracts/side/entry_price(average)` 常见
- 数量与精度：先上调至最小名义/数量，再下单；余额不足（如 43012）走“数量递减重试”


## 常见问题（FAQ）

  - 查看 `risk_blocked` 事件与 `record_risk_event` 详情（名义/余额/日限额等）
  - 调整 `RISK_PERCENTAGE`、`margin_amount`、或降低杠杆
- 止损下单失败？
  - 确认交易所接口与参数（Bitget 需专用 TPSL；单向持仓/holdSide、productType/marginCoin）
  - 查看结构化日志与返回 JSON
- 分批止盈没有执行？
  - 价格型 TP1 生效后会自动挂 TP2/TP3；无价格提示则使用回退三档
  - 若后续收到价格型 TP1，会先撤回回退挂单再挂价格型
- PnL 为什么与预期不同？
  - 以真实 `exit_price` 计算：`(exit-entry)*size*leverage`（空单反向）
  - 关注手续费与交易所四舍五入/精度截断
- Telegram 断线？
  - 已配置无限重连；检查网络/代理，必要时重启 GUI/进程
